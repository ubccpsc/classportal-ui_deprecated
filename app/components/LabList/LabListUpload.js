...
const API_CLASS_LIST_PROPERTY = 'labList';

class LabListUpload extends React.Component {
  constructor(props) {
    super(props);
    this.state = { files: [] };
  ...
  uploadForm(e) {
    ...
    if (!this.state.files.length) {
      ...
      // POST api/class/list
      this.props.dispatch(labListActions.uploadLabList(this.courseNum, csv))
        .then((response) => {
          console.log(`LabListUpload::uploadLabList(courseNum) SUCCESS: ${response}`);
        })
        .catch(console.log(`LabListUpload::uploadLabList(courseNum) ERROR: ${response}`));
    }
  }
  ...
  render() {
    ...
    if (!this.props.user) {
      ...
      return (
        <Module id="upload-classlist-module" title="Upload New LabList" initialHideContent={false}>
...
export default connect(mapStateToProps)(LabListUpload);
